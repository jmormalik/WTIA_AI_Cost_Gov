...
6. **Assemble pitch deck (.pptx)** — full 16-slide outline DONE (7/24) in `Pitch/피치덱-통합-아웃라인.md` (paste-ready). Remaining = (a) drop into Gamma/Slides, (b) fill 6 gaps flagged in the outline: product/team name, traction slide real data, price/finance ⚠️ numbers (Q-P2 interviews), Ask amount, embed dashboard demo, team "why us" line. Demo to embed = `deliverables/DEMO_최신_이걸_더블클릭.html` (ready, offline-safe).
